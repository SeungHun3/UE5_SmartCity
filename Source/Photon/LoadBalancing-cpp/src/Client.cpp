...
#	pragma warning(disable:4355)
			...
			, mUseUDPEncryption(connectionProtocol==ConnectionProtocol::UDP&&mUseAuthOnce)
#elif !defined _EG_ENCRYPTOR_AVAILABLE || !defined _EG_WEBSOCKET_AVAILABLE
...
#	pragma GCC diagnostic pop
		...
		/** @fn template<typename Ftype> int Client::sendDirect(const Ftype pParameterArray, typename Common::Helpers::ArrayLengthType<Ftype>::type arrSize, const Common::JVector<int>& targetPlayers, bool fallbackRelay)
		   @overload
		...
		/** @fn template<typename Ftype> int Client::sendDirect(const Ftype pParameterArray, const short* pArrSizes, const Common::JVector<int>& targetPlayers, bool fallbackRelay)
		   @overload
			...
			case OperationCode::AUTH_ONCE:
				{
					...
					if(operationResponse.getReturnCode())
					{
						...
						case ErrorCode::OPERATION_DENIED:
							...
							break;
						case ErrorCode::CLIENT_VERSION_TOO_OLD:
							mDisconnectedCause = DisconnectCause::CLIENT_VERSION_TOO_OLD;
							break;
						case ErrorCode::CLIENT_VERSION_INVALID:
							mDisconnectedCause = DisconnectCause::CLIENT_VERSION_INVALID;
							break;
						case ErrorCode::DASHBOARD_VERSION_INVALID:
		...
		{
			EGLOG(eventData.getCode()>EventCode::MAX_CUSTOM_EVCODE&&eventData.getCode()!=EventCode::PUNCH_MSG&&eventData.getCode()!=EventCode::VOICE_DATA?DebugLevel::INFO:DebugLevel::ALL, L"%ls", eventData.toString().cstr()); // don't print out the payload here, as that can get too expensive for big events
		...
		{
			if(mUseAuthOnce)
				return mpPeer->opAuthenticateOnce(mAppID, mAppVersion, M_CONNECTION_PROTOCOL, mUseUDPEncryption?EncryptionMode::UDP_TRAFFIC_ENCRYPTION_RANDOM_SEQUENCE_GCM:EncryptionMode::USER_DATA_ENCRYPTION_NO_DIFFIE_HELLMAN, mAuthenticationValues, mAutoLobbyStats, mSelectedRegion);
			else
